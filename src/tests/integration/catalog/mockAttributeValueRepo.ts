import type { ISkuAttributeValueRepository } from "@modules/catalog/application/interfaces/repository/ISkuAttributeValueRepository";
import type { SkuAttributeValue } from "@modules/catalog/domain/entities/sku-attribute-value";


export class InMemorySkuAttributeValueRepository implements ISkuAttributeValueRepository {
  private items: SkuAttributeValue[] = [];

  async findById(id: string): Promise<SkuAttributeValue | null> {
    const item = this.items.find(item => item.id === id);
    return item || null;
  }

  async findBySkuAndAttribute(skuId: string, attribute_id: string): Promise<SkuAttributeValue | null> {
    const item = this.items.find(
      item => item.skuId === skuId && item.attribute_id === attribute_id
    );
    return item || null;
  }

  async findAllBySku(skuId: string): Promise<SkuAttributeValue[]> {
    return this.items.filter(item => item.skuId === skuId);
  }

  async save(entity: SkuAttributeValue): Promise<void> {
    this.items.push(entity);
  }

  async update(entity: SkuAttributeValue): Promise<void> {
    const index = this.items.findIndex(item => item.id === entity.id);
    if (index !== -1) {
      this.items[Number(index)] = entity;
    }
  }

  async delete(id: string): Promise<void> {
    const index = this.items.findIndex(item => item.id === id);
    if (index !== -1) {
      this.items.splice(index, 1);
    }
  }
}
